...
def english_app(sandbox_env):
    ...
    **改配置再启动**更短也更稳，而且它顺带覆盖了「配置里的语言在启动时就生效」这条真实路径。
    ...
    #
    # 插件系统在启动约 2 秒后**后台**同步官方模块（`ScheduleOfficialPluginSync`）。同步一旦
    # 赶在采集之前完成，插件页就会多出「已安装模块」卡片（`🗑 卸载` / `启用` …）；赶不上就没有。
    # 于是同一份代码会给出**时多时少**的观测集合 ⇒ 台账时红时绿。而一个会随机红的用例，
    # 最大的代价不是它红了，是它训练所有人无视红灯。
    #
    # 本用例断言的是**应用自身的文案**，与远端目录里恰好有哪些模块无关，所以掐网是对的。
    # 做法：代理指向一个必然拒绝连接的本地端口。实测 .NET 在 Windows 上同样优先读
    # `HTTP(S)_PROXY`，应用日志立即出现「刷新官方插件目录失败：目标计算机积极拒绝 (127.0.0.1:1)」，
    # 且失败路径本就**只记日志、不阻塞首帧** —— 顺带让本用例在离线 / CI 环境同样成立。
    env = dict(env)
