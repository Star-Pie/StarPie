...
2. **``tab_4``（关于 / 更新日志）不在范围内**：该页正文是**发行说明散文**，
   ...
   那会训练所有人去改台账，台账就失去了权威。代价是这一页的**标题类文案**
   （如「版本演进里程碑」）也一并失去覆盖，属已知欠账。
...
6. **``tab_6``（官方插件市场）只有"壳"在覆盖内**。同一份夹具原因：本用例把网络掐断了
   （见 ``english_app`` 夹具里那段注释），官方 catalog 拿不到 ⇒ 双列网格里一张卡都不渲染
   ⇒ ``ItemTemplate`` 从未实例化。被采到的只有页头 / 说明 / 搜索框 / 四个筛选标签 /
   状态行 / 空状态 —— 也就是这一页**由窗口类逐项重设**的那批控件，恰好是漏接的重灾区，
   所以这一页进范围仍然是划算的；卡片上的文案（模块名 / ID / 状态徽标 / 安装按钮）
   与 ``tab_5`` 的卡片同理，靠 ``--plugin-selftest`` 守着。
...
#
# `tab_6`（官方插件市场）加进来是有意的：它是新增页，整页文案都已经接好 i18n，
# 所以它的桶**应当采到空集**（英文界面上一句中文都没有）。空集天然是台账的子集，
# 因此不需要为它重采一次台账；一旦有人往这一页写死一句中文，它就会当场变红。
# 这正好补上「新页没人守」的空档。
_IN_SCOPE_TABS = (0, 1, 2, 3, 5, 6)

# 侧边栏页签按钮（NavTab0..NavTab6）是**已接好 i18n** 的控件：英文界面下必须无中文。
