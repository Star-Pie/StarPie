...
## [v1.8.1-beta.1] - 2026-09-25

本测试版为官方「按键映射」插件提供 SDK 1.7 宿主能力。插件需由用户在「插件与扩展」中单独下载安装，不随主程序发行包附带。

### 新增
- 轮盘动作可切换自定义键盘映射层；支持按键录制、可视化关系图、点击源键与目标键直接新增映射，以及画布缩放与适配。
- 映射层在焦点切换、停用与紧急退出时释放；未映射按键保持原样通过。
- 插件 SDK 新增键盘重映射服务与 `KeyMap` 参数类型，供官方插件使用。

### 修复
- 修复有效映射已配置时，动作设置仍错误显示「必填参数留空」的提示；缺失或非法映射继续显示真实校验结果。
- 修复官方保留 ID 插件在清单扫描路径中的识别问题，并为隔离测试实例加入开机启动项保护。
