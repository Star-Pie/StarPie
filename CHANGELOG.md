...
- 已安装插件卡片的操作组新增 **⚡ 分配至轮盘**：一键完成「跳到手势页 → 选定扇区 →
  动作类型设为 🔌 插件动作 → 选中该插件的动作」，并提示去右侧精调参数。
  扇区选择有明确顺序：**优先当前选中的扇区 → 其次第一个空扇区 → 全满才覆盖当前扇区**
  （覆盖时提示会说出被替换掉的动作名）。
- 判据是「**认领型还是动作型**」，不是「已登记动作数」：12 个官方模块的动作全部以
  顶层动作类型提供（类型下拉里直接选），对它们 ⚡ 保持禁用、气泡说明「不需要分配」；
  只有走 `Type=Plugin + PluginActionRef` 通道的社区插件才亮起。判据与禁用原因由纯函数
  `PluginWheelAssignment.ClassifyBlock` 给出，界面只负责把原因翻成词条。
- **点之前先把这一个插件拉起来**（惰性加载契约）：原先卡片上「贡献 N 个动作」按快照算，
  插件未加载时恒为 0，⚡ 会永远灰着。现写入前经 `PluginHost.EnsurePluginLoadedForUi`
  按需加载（新增 `PluginActivationReason.WheelAssignment`），加载失败的原因映射成本地化提示。
  加载成功后卡片状态徽章与 meta 行同步刷新（「已启用 · 待加载」→ ✅「运行中」）。
- 卡片 meta 行补一档「认领 N 个顶层动作类型」：未加载 + 有认领时不再误说「暂无动作」。
- 自检 `[3f]` 新增合成数据断言：0 / 1 / N 动作三个分支、有空位不许覆盖、
  覆盖时必须报出被替换的动作名、四类禁用原因的词条两两分叉（英文宿主部分无 CJK 同步覆盖）。
  **三条变异测试验过会红**（判据回退成已登记数 / 两种气泡写成同一句 / 未加载时多说一句）。
- 端到端实测（沙箱 + 真实登记表）：点击后切到手势页、扇区 1 动作类型 = 🔌 插件动作、
  动作下拉选中目标动作、托盘气泡报出「已分配到『…』」。
- 纯界面与加载时机改动：**未触碰动作分派逻辑与配置格式**。

### 🎨 安装确认页改为自绘分区窗口

- 原先是一次 `MessageBox.Show(纯文本)`，十来行等权文字挤在一个滚动区里。改为自绘窗口
  `PluginInstallDialog`：六个分区逐块成卡，能力行逐条加 ✔、风险行加 ⚠️，
  按钮为「复制详情 / 取消 / 安装」。
- **风险提示固定在「安装」按钮上方**（`PluginsConfirmSecurityTitle` 提为常量，窗口与自检共用）。
  第一版全塞滚动区，实截图发现「安全提示」被滚出可视区——用户按下前最后看到的是文件哈希
  而不是「它会以你的权限运行代码」。现在正文可滚、风险常驻。
- 窗口高度按屏幕可用高度自适应：正文是唯一的 `*` 行，被压缩时自己出滚动条，
  而不是把按钮挤出屏幕。深 / 浅主题与英文界面均实测。
- `PluginInstallConfirmation` 重写为「分区 + 展平」双消费形态：`BuildSections` 供窗口逐区渲染，
  `Build` 展平供「复制详情」与自检 `[3e]` 字符串断言，信息同源。`[3e]` 新增第 ⑦ 段
  分区结构断言（分区数 / 标题互异 / 能力逐条成行 / 末段必为安全提示），两条变异测试验过会红。
- 顺带修一处**判据盲区**：`scratch/check_i18n.py` 的漏接正则「一个元素只报第一个含中文属性」，
  同一元素上的 `ToolTip` 落在消费区间里再也不会被看到——插件页 5 处 ToolTip 因此长期
  被判为已接（`Content` 接了、`ToolTip` 没接）。改为逐属性解析，并新增全仓 XAML 漏接棘轮
  （存量 127 条登记 `scratch/i18n_xaml_leak_baseline.json`，新增即红；植入探针的变异测试
  验证同一元素的 `Content` 与 `ToolTip` 会同时被报出）。

### 🧩 官方插件市场独立成页

- 插件页内嵌的官方插件面板迁出，新增独立页签 **「官方插件市场」（`NavTab6`，侧边栏末位）**：
  双列卡片网格（头像 / 名称 + 版本 + 状态徽章 / 模块 ID / 认领标签 / SHA-256 指纹 / 安装按钮）、
  搜索框（命中名称或模块 ID）、四个筛选（全部 / 未安装 / 已安装 / 可更新）、计数行与空状态。
  空状态只在「目录已拿到且筛完为空」时出现——目录还没到就报「没有匹配」是答非所问。
- 目录拉取时机搬移：进入市场页才联网拉取。原先挂在插件页刷新里，面板搬走后进度行会在
  一个看不见的页面上空转。
- 修掉三处摊开页面才看得见的缺陷：
  ① 进度行永远停在「正在获取…」（渲染发生在 `finally` 之前，`_officialPluginsLoading`
  还是 true ⇒ 成功看不到目录版本、失败看不到原因。提取 `SetOfficialPluginsLoading`
  把「置标志 + 重渲染」绑成一个动作，渲染只在标志回落之后发生）；
  ② 侧边栏底部四个主题段名（系统 / 浅色 / 曜黑 / 钛灰）是无名控件，**四个页签各漏 4 条、
  台账里挂了 20 条**的中文泄漏——加名 + 接线 + 台账收紧一次完成；
  ③ 双列网格卡片被 `UniformGrid` 平分视口高度撑成一大块空白（`VerticalAlignment="Top"`）。
- 折叠侧边栏逻辑适配带徽章的新页签（`Grid` 型内容 + 徽章归位 + 分组标题收起），
  七页签切换用例扩到 7 页；`i18n_baseline.json` 新增 `tab_6` 空桶。
