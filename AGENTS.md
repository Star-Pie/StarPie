...
- **`PluginSelfTest.cs` 的段落号是结构契约，不是装饰。** 它承载 `[0]`…`[7]`（含 `[3b]`/`[3c]`/`[3d]`/`[3e]`/`[3f]`/`[3g]`/`[3j]`）共十六段断言，而本文件是全仓**唯一**没有单测保护的执行体 —— 它自己就是验证手段。任何「整文件重写」或「解决冲突整体取一侧」都可能在无人察觉的情况下整段顶掉断言。
  ...
  - **`[3f]` 守「插件管理页卡片文案」**（2026-09-19 加）。这一段的存在理由是**别的手段都够不着**：卡片在 `ListBox.ItemTemplate` 里，`DataTemplate` 命名域不同 ⇒ `Name` 无效（静态差集的「具名控件漏接 = 0」看不见）；而用例沙箱里**没有已安装插件** ⇒ 列表为空、模板从未实例化（台账也看不见）。所以「卡片翻没翻」只能靠**真的构建一次卡片**来断言：① 逐 `PluginRuntimeState` 成员驱动 `PluginListItem.DescribeState`，断言非空、**不是裸键名**、有图标，并单独判 `Active`/`Installed` 的两种处境文案不同；② 逐语言 `PluginListItem.Build`，英文卡片的**宿主部分**（先按**长度降序**摘掉插件自带数据）不许有方块字与全角标点；③ 详情里必须含 `pluginId`、meta 行非空（否则「没有中文」可能只是「什么都没拼」）；④ 能力标签**逐项**非空、不是裸键名，且**非英文语言下不得出现未翻译的英文枚举名**；⑤ 四语言卡片两两不同。段落刻意排在 `[4]` **之前** —— `--skip-invoke` 在 `[4]` 开头提前 return，排后面等于日常回归不执行。
    - **卡片每加一层 / 一个字段，就要同步纳入这段断言**（2026-09-20 卡片由四层扩到六层时，头像 / 描述 / meta 行 / 能力标签四项一并纳入）。新字段不纳入 = 它在英文界面泄漏中文无人看守 —— 而这类泄漏的形态正是**编译能过、静态检查全绿、界面上就是错的**。
    - **断言的覆盖面不许依赖「沙箱里那个插件恰好声明了什么」**。能力标签那段最初只走「构建真实卡片」一条链路，而沙箱插件的 `CapabilitiesAck` 为空时整段静默跳过、只有一项时「不认识的能力名」那条分支永远跑不到。因此把 `PluginListItem.BuildCapabilityTags` 从 `private` 提成 `internal`，由自检**用合成数据直接驱动**（`Process` / `WindowControl` / `NotARealCapability` 覆盖「认识的 / 不认识的」两条分支）。**两条链路是分工不是重复** —— 变异测试验过：退回「原样显示英文枚举名」由卡片级断言报红；把不认识的名字静默丢弃则由合成那条报红（卡片级抓不到，沙箱插件声明的都是认识的名字）。
  ...
  - **`[3e]` 守「确认页整页随语言切换」**（2026-09-19 加）。安装确认页是最不能含糊的一页（用户在这里决定要不要让这段代码在自己电脑上跑），而它的正文里唯一的非词条来源是清单字段与文件路径 —— 所以自检用**合成**的扫描结果（全 ASCII）构造正文，逐语言断言：① 无未替换的 `{n}` 占位符；② **英文页里一个方块字、假名或全角标点都不许有**；③ 四种语言产生四份互不相同的正文；④ 关键字段真的拼进去了；⑤ 两种「装完是否立即启用」产生不同正文。另有一条守「每个状态位都有安装后果文案」，判据写成「共用兜底措辞的状态集合正好是哪几个」—— 新增状态位忘了配文案时会落进那个集合，断言随即变红（写成列举式的话，新状态位根本没机会被这条断言看见）。2026-09-20 确认页改为自绘分区窗口（`PluginInstallDialog`）后追加了第 ⑦ 段：`BuildSections` 的分区数 ≥ 5、标题键互不相同、能力逐条成行、**末段必为 `SecuritySectionKey`（安全提示）** —— 这条与窗口布局共用同一个常量：窗口靠它把风险段摘出来固定到「安装」按钮上方，正文可滚、风险常驻；自检靠它断言顺序。改分段顺序或动这个常量，两处会同时被问到。
...
**`check_i18n.py` 另有一条「具名控件逐属性」棘轮**（2026-09-20 加，`scratch/i18n_xaml_leak_baseline.json`，存量 127 条，新增即 EXIT=1）：
全仓 XAML 里**带 `Name` 且含中文**的 `Text`/`Content`/`ToolTip` 等属性，若代码从未重设该属性则记入台账。
起因是一个判据盲区的实例：旧漏接正则用 `[^>]*?` 非贪婪横扫，**同一元素只报第一个**含中文属性——
插件页 5 处 `ToolTip` 因此长期被判为「已接」（`Content` 接了、`ToolTip` 没接），三个护栏全绿、界面上就是错的。
**教训**：属性级漏接要用「开标签 → 标签内逐属性」解析，别用跨属性的非贪婪正则；这条棘轮与上面的运行时台账是分工
—— 它守「有 `Name` 但漏接某个属性」，运行时台账守「整页有没有翻」。
